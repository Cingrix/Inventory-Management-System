...
export const useMutations = (apiUrl, view, itemsPerPage, currentPage, searchTerm, sortBy, sortOrder, filters) => {
  ...
  const addMutation = useMutation({
    mutationFn: (values) => axios.post(`${apiUrl}/api/items`, values,),
    onSuccess: (res) => {
      if (view === 'table') {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, updatedItem }) => axios.put(`${apiUrl}/api/items/${id}`, updatedItem),
    onSuccess: (res) => {
      if (view === 'table') {
        queryClient.setQueryData(['items', currentPage, searchTerm, sortBy, sortOrder, filters], (oldData) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axios.delete(`${apiUrl}/api/items/${id}`),
    onSuccess: () => {
      if (view === 'table') {
        queryClient.setQueryData(['items', currentPage, searchTerm, sortBy, sortOrder, filters], (oldData) => {
  ...
  const transferMutation = useMutation({
    mutationFn: async ({ itemId, fromWarehouse, toWarehouse, quantity }) => {
      const res = await axios.post(`${apiUrl}/api/items/transfer`, { itemId, fromWarehouse, toWarehouse, quantity });
      return res.data;
    },
  ...
  const warehouseMutation = useMutation({
    mutationFn: async (values) => {
      if (values._id) {
        const res = await axios.put(`${apiUrl}/api/warehouses/${values._id}`, values);
        return res.data;
      } else {
        const res = await axios.post(`${apiUrl}/api/warehouses`, values);
        return res.data;
      }
    },
    onSuccess: () => {
  ...
  const deleteWarehouseMutation = useMutation({
    mutationFn: async (id) => {
      const res = await axios.delete(`${apiUrl}/api/warehouses/${id}`);
      return res.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['warehouses']);
  ...
  const logMutation = useMutation({
    mutationFn: (logData) => axios.post(`${apiUrl}/api/logs`, logData),
